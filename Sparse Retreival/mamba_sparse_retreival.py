...
def get_prompt_without_context(columns, context):
    input_prefix = f"Question: {columns[1]}\nA: {columns[2]}\nB: {columns[3]}\nC: {columns[4]}\nD: {columns[5]}\nE: {columns[6]}"
...
for num_retireved in [0, 1, 3, 6, 10]:
    predictions = []
    submit_ids = []
    start = time.time()
    for index in tqdm(range(df_valid.shape[0])):
        ...
        context = ""
        for j in range(num_retireved, 0, -1):
            context += f"{retrieved_articles_parsed[index][-j][2]}\n"

        if num_retireved != 0:
            inputs = tokenizer(get_prompt_with_context(columns, context), return_tensors="pt").to(f"cuda:{model.device.index}")
        else:
            inputs = tokenizer(get_prompt_without_context(columns, context), return_tensors="pt").to(f"cuda:{model.device.index}")
        with torch.no_grad():
            outputs = model(**inputs)
